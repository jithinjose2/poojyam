...
if($is_draw){
    $back_image = BASE.'img/share_draw.jpg';
}else{
    $back_image = BASE.'img/share_winner.jpg';
}

$font       = BASE."fonts/Melbourne-Bold.ttf";
$font_mal   = BASE."fonts/AnjaliOldLipi.ttf";

$image_1 = imagecreatefromjpeg($back_image);
...
foreach($temp as $player){
    ...
    $details1 = calculateTextBox($font_size,0,$font_mal,ucwords($player['name']));
    imagettftext($image_1, $font_size, 0, $details1['left']+60, $details1['top']+140+$i,$color2,$font_mal,ucwords($player['name']));
    
    $details1 = calculateTextBox($font_size,0,$font_mal,$player['score']);
    imagettftext($image_1, $font_size, 0, $details1['left']+235, $details1['top']+140+$i,$color2,$font_mal,$player['score']);
    $i+=30;
}
...
if(!$is_draw){
    $details1 = calculateTextBox(18,0,$font_mal,ucwords($winner_name));
    imagettftext($image_1, 18, 0, $details1['left']+60, $details1['top']+280,$color2,$font_mal,ucwords($winner_name));
}
...
for($i=0;$i<$game['size'];$i++){
    for($j=0;$j<$game['size'];$j++){
        if(isset($boxes["$i,$j"])){
            ...
            imagettftext($image_1, 20, 0, $left+330, $top-55,$plyer['color_m'],$font_mal,$first_letter);
        }
    }
}
